...
function parseConfig(config) {
  if (!config || !config.api) {
    console.log(`Missing api configuration, authentication features disabled.`);
    return;
  }
  if (!config.api.me || !config.api.roles) {
    console.log(`Missing API configuration for me and/or roles, authentication features disabled.`);
    return;
  }
  return {
